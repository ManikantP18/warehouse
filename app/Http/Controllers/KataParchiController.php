...
class KataParchiController extends Controller
{
    public function index(){
        ...
        $data['kataparchi'] = DB::select('select kata_parchi.*,product_services.name as kp_verity  from kata_parchi join branches on branches.branch_id  = kata_parchi.kp_godown_name join product_services ON kata_parchi.kp_verity = product_services.id where kata_parchi.is_deleted = 0 AND kp_pure_wigth = 0');
